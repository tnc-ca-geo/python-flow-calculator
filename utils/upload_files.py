...
from utils.constants import DELETE_INDIVIDUAL_FILES_WHEN_BATCH, QUIT_ON_ERROR, NUMBER_TO_CLASS, PRODUCE_DRH, OUTPUT_FILE_DIR
...
def upload_files(start_date, gage_arr, output_files = OUTPUT_FILE_DIR, batched = False, alteration_needed = False, aa_start_year = None, aa_end_year = None):
    ...
    metadata_files = []
    file_identifiers = []
    file_base_name = ['annual_flow_result']

    i = 0
    for gage in gage_arr:
        print(f"starting metric computation on comid:{gage.comid} {i/len(gage_arr) * 100}% complete this parquet file")
        i = i+1
        ...
        try:
            ...
            output_dir1 = write_annual_flow_result(file_name, results, file_base_name[0])
            if PRODUCE_DRH:
            ...
            output_file_dirs[0].append(output_dir1)
    ...
    return output_file_dirs[0], warning_message
...
def batch_metadata_files(metadata_file_paths, file_identifier, output_dir):
    ...
    combined_data.to_parquet(os.path.join(output_dir, "combined_metadata.parquet"), index=False)
...
def batch_files(file_paths, base_file_name, file_identifier, output_dir, alteration_needed):
    ...
    combined_data = combined_data[column_order]
    print(f'Writing output to {os.path.join(output_dir, "combined_" + base_file_name + ".parquet")}')
    combined_data.to_parquet(os.path.join(output_dir, "combined_" + base_file_name + ".parquet"), index=False)
